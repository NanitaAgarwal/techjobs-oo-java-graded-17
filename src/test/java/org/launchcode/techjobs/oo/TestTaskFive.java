...
public class TestTaskFive extends AbstractTest {
    ...
    public void testToStringStartsAndEndsWithNewLine() throws ClassNotFoundException, NoSuchMethodException, InvocationTargetException, InstantiationException, IllegalAccessException {
        ...
        String firstChar = String.valueOf(job.toString().charAt(0))+String.valueOf(job.toString().charAt(1));
        String lastChar = String.valueOf(job.toString().charAt(job.toString().length()-2))+String.valueOf(job.toString().charAt(job.toString().length()-1));
